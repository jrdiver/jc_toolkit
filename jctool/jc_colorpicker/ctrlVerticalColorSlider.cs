...
public class ctrlVerticalColorSlider : UserControl
{
    ...
    private void ClearSlider()
    {
        ...
        SolidBrush brush = new(Color.FromArgb(70, 70, 70));
    ...
    private void DrawSlider(int position, bool Unconditional)
    {
        ...
        Pen pencil = new(Color.FromArgb(116, 114, 106));    //	Same gray color Photoshop uses
        Brush brush = Brushes.White;

        Point[] arrow = new Point[7];               //	 GGG
        arrow[0] = new(1, position);            //	G   G
        arrow[1] = new(3, position);            //	G    G
        arrow[2] = new(7, position + 4);        //	G     G
        arrow[3] = new(3, position + 8);        //	G      G
        arrow[4] = new(1, position + 8);        //	G     G
        arrow[5] = new(0, position + 7);        //	G    G
        arrow[6] = new(0, position + 1);        //	G   G
        //	 GGG
        ...
        //	    GGG
        arrow[0] = new(Width - 2, position);        //	   G   G
        arrow[1] = new(Width - 4, position);        //	  G    G
        arrow[2] = new(Width - 8, position + 4);    //	 G     G
        arrow[3] = new(Width - 4, position + 8);    //	G      G
        arrow[4] = new(Width - 2, position + 8);    //	 G     G
        arrow[5] = new(Width - 1, position + 7);    //	  G    G
        arrow[6] = new(Width - 1, position + 1);    //	   G   G
        //	    GGG
    ...
    private void Draw_Style_Hue()
    {
        ...
        for (int i = 0; i < Height - 8; i++)    //	i represents the current line of pixels we want to draw horizontally
        {
            ...
            Pen pen = new(AdobeColors.HSL_to_RGB(_hsl));    //	Get the Color for this line
    ...
    private void Draw_Style_Red()
    {
        ...
        for (int i = 0; i < Height - 8; i++) //	i represents the current line of pixels we want to draw horizontally
        {
            int red = 255 - (int)Math.Round(255 * (double)i / (Height - 8));    //	red is based on the current vertical position
            Pen pen = new(Color.FromArgb(red, m_rgb.G, m_rgb.B));   //	Get the Color for this line
    ...
    private void Draw_Style_Green()
    {
        ...
        for (int i = 0; i < Height - 8; i++) //	i represents the current line of pixels we want to draw horizontally
        {
            int green = 255 - (int)Math.Round(255 * (double)i / (Height - 8));  //	green is based on the current vertical position
            Pen pen = new(Color.FromArgb(m_rgb.R, green, m_rgb.B)); //	Get the Color for this line
    ...
    private void Draw_Style_Blue()
    {
        ...
        for (int i = 0; i < Height - 8; i++) //	i represents the current line of pixels we want to draw horizontally
        {
            int blue = 255 - (int)Math.Round(255 * (double)i / (Height - 8));   //	green is based on the current vertical position
            Pen pen = new(Color.FromArgb(m_rgb.R, m_rgb.G, blue));  //	Get the Color for this line
    ...
    private void Reset_Slider(bool Redraw)
    {
        ...
        {
            case eDrawStyle.Hue:
                m_iMarker_Start_Y = Height - 8 - (int)Math.Round((Height - 8) * m_hsl.H);
                break;
            case eDrawStyle.Saturation:
                m_iMarker_Start_Y = Height - 8 - (int)Math.Round((Height - 8) * m_hsl.S);
                break;
            case eDrawStyle.Brightness:
                m_iMarker_Start_Y = Height - 8 - (int)Math.Round((Height - 8) * m_hsl.L);
                break;
            case eDrawStyle.Red:
                m_iMarker_Start_Y = Height - 8 - (int)Math.Round((Height - 8) * (double)m_rgb.R / 255);
                break;
            case eDrawStyle.Green:
                m_iMarker_Start_Y = Height - 8 - (int)Math.Round((Height - 8) * (double)m_rgb.G / 255);
                break;
            case eDrawStyle.Blue:
                m_iMarker_Start_Y = Height - 8 - (int)Math.Round((Height - 8) * (double)m_rgb.B / 255);
                break;
        }
    ...
    private void ResetHSLRGB()
    {
        switch (m_eDrawStyle)
        {
            ...
            case eDrawStyle.Brightness:
                ...
                break;
            case eDrawStyle.Red:
                m_rgb = Color.FromArgb(255 - (int)Math.Round(255 * (double)m_iMarker_Start_Y / (Height - 9)), m_rgb.G, m_rgb.B);
                m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
                break;
            case eDrawStyle.Green:
                m_rgb = Color.FromArgb(m_rgb.R, 255 - (int)Math.Round(255 * (double)m_iMarker_Start_Y / (Height - 9)), m_rgb.B);
                m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
                break;
            case eDrawStyle.Blue:
                m_rgb = Color.FromArgb(m_rgb.R, m_rgb.G, 255 - (int)Math.Round(255 * (double)m_iMarker_Start_Y / (Height - 9)));
                m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
                break;
        }
    }
