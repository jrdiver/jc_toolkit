...
/******************************************************************/
/*****                                                        *****/
/*****     This version is heavily optimized for use in       *****/
...
/// </summary>
public class JoyConColorPicker : Form
{
    ...
    private Color m_rgb;
    private bool errorCreatingPresets;
    private bool gripsColor;
    private bool showGripsPanel;
    ...
    public Button m_cmd_OK;
    public Button m_cmd_Cancel;
    private TextBox m_txt_Hue;
    private TextBox m_txt_Sat;
    private TextBox m_txt_Black;
    ...
    private RadioButton m_rbtn_Black;
    ...
    private Label m_lbl_New_Color;
    private Label m_lbl_Old_Color;
    ...
    private Label m_lbl_Black_Symbol;
    ...
    public JoyConColorPicker(Color starting_color, Color starting_color2, bool gripColorOptionsOn)
    {
        ...
        m_eyedropColorPicker.MouseUp += OnEyeDropperSelectionChanged;
    ...
    {
        System.Xml.XmlDocument olddoc = new();
        try
        {
    ...
    public void loadGripsPanel()
    {
        if (gripsColor)
        {
            m_radio_btn_Body.Text = "Grip (L)";
            m_radio_btn_Buttons.Text = "Grip (R)";
            showGripsPanel = true;
            btn_switchGripsPanel.Text = "Retail";
        }
        else
        {
            panel2.Controls.Remove(panel_RetailGripColors);
    ...
    private void m_lbl_OldColor_Click(object sender, EventArgs e)
    {
    ...
    private void m_lbl_OldBtnColor_Click(object sender, EventArgs e)
    {
    ...
    private void Validate_Color_TextValues(object sender, KeyEventArgs e)
    {
        ...
        if (text.Any(letter => !char.IsNumber(letter)))
        {
            UpdateTextBoxes();
            WriteHexData(m_rgb);
            return;
    ...
    private void m_lbl_Preset_Click(object sender, EventArgs e)
    {
        Buttons.RoundButton senderButton = (Buttons.RoundButton)sender;

        UncheckPreset(panel2);
    ...
    private void SetPresetColorName(object sender, EventArgs e)
    {
        ...
        foreach (Control child in panel_RetailUserColors.Controls)
        {
            ...
            {
                PresetNameDialog presetDialog = new();
                presetDialog.txtbox_presetName.Text = toolTip1.GetToolTip(rB);
                if (presetDialog.ShowDialog(this) == DialogResult.OK)
                {
                    // Read the contents of presetNameDialog's TextBox.
                    rB.BackColor = m_rgb;
                    toolTip1.SetToolTip(rB, presetDialog.txtbox_presetName.Text);
    ...
    private void ClearPreset(object sender, EventArgs e)
    {
        // Check Retail user colors
        foreach (Control child in panel_RetailUserColors.Controls)
        {
    ...
    private void btn_switchGripsPanel_Click(object sender, EventArgs e)
    {
        ...
        {
            showGripsPanel = false;
            btn_switchGripsPanel.Text = "Grips";
            panel2.Controls.Remove(panel_RetailGripColors);
        }
        else
        {
            ...
            AutoScaleDimensions = new(96, 96);
            btn_switchGripsPanel.Text = "Retail";
            panel2.Controls.Add(panel_RetailGripColors);
            panel_RetailGripColors.BringToFront();
        }
    }
            ...
            foreach (Control child in Util.controlsSorted(panel_RetailUserColors))
            {
                ...
                if (rB != null)
                {
                    ...
                    if (att.Value == "retailpreset_" + (i + 1).ToString("D2"))
                    {
                        ...
                        if (att != null)
                            toolTip1.SetToolTip(rB, att.Value.ToString());
                    }
                    i++;
                }
            }
    ...
    private void convertOldFormatCustomConfig()
    {
        ...
        foreach (Control child in Util.controlsSorted(panel_UserColors))
        {
            ...
            if (rB != null)
            {
                ...
                toolTip1.SetToolTip(rB, "Old Preset " + (j + 1).ToString());
            }
            j++;
        }
        try
    ...
    private void saveCustomColorToConfig()
    {
        ...
        foreach (Control child in Util.controlsSorted(panel_RetailUserColors))
        {
            ...
            if (rB != null)
            {
                ...
                xe.SetAttribute("tooltip", toolTip1.GetToolTip(rB));
                node.AppendChild(xe);
            }
        }
        try
    ...
    private uint swapBytesColor(uint x)
    {
        x = x << 8;
    ...
    private void SetColorBasedOnType(Color m_RGB_color)
    {
        ...
        {
            m_lbl_New_Color.BackColor = m_RGB_color;
            m_lbl_New_Color.Update();
        }
        else
        {
            m_lbl_New_ButtonColor.BackColor = m_RGB_color;
            m_lbl_New_ButtonColor.Update();
        }
    }
    ...
    {
        get => m_lbl_New_ButtonColor.BackColor;
        set
        {
            m_rgb = value;
            m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
